...
# --------------------------
# MODELS
from models.resnetGeno import resnet18 as resnet18MTL
...
if __name__ == '__main__':
    ...
    dirWorkspace1Test = 'D:/Workspace/DB HTT - Public (test) (Mahdi)/'
    dbName1 = 'ADP'
    ...
    if not os.path.exists(dirDb1Test):
        os.mkdir(dirDb1Test)
    ...
    for trainMode in trainModes:
        ...
        for hierarNum in range(2, 3):
            ...
            for i, (modelData) in enumerate(modelNamesAll):
                ...
                # dir results
                dirResult = './results/' + trainMode + '/' + 'level_{}'.format(hierarNum+1) + '/' + modelData['name'] + '/'
                ...
                for r in range(0, num_iterations): #(nFolds-1)
                    ...
                    if modelData['name'] == 'resnet18':
                        ...
                        if trainMode == 'imagenet':
                            currentModel = resnet18MTL(pretrained=True)
                            # fine tune
                            for param in currentModel.parameters():
                                param.requires_grad = False  # False frozen for warm-up

                        if trainMode == 'scratch':
                            currentModel = resnet18MTL(pretrained=False)
                            # fine tune
                            for param in currentModel.parameters():
                                param.requires_grad = False  # False frozen for warm-up
                        ...
                        # Multi-task learning
                        # fc1
                        new_classifier1 = nn.Linear(modelData['sizeFeatures'], num_classes)
                        currentModel.fc1 = new_classifier1
                        # fc2
                        new_classifier2 = nn.Linear(modelData['sizeFeatures'], 2)
                    ...
                    # ------------------------------------------------------------------- transform db
                    # dLabel = util.dbToDataStore(dirDb1Orig, dirDb1Test, extOrig, extNew, log)
                    dLabel = 'dummyLabel'
                    ...
                    if not os.path.exists(dirOutTrainTest1):
                        os.mkdir(dirOutTrainTest1)
                        os.mkdir(os.path.join(dirOutTrainTest1, 'train'))
                        os.mkdir(os.path.join(dirOutTrainTest1, 'val'))
                        os.mkdir(os.path.join(dirOutTrainTest1, 'test'))
                        os.mkdir(os.path.join(dirOutTrainTest1, 'train', dLabel))
                        os.mkdir(os.path.join(dirOutTrainTest1, 'val', dLabel))
                        os.mkdir(os.path.join(dirOutTrainTest1, 'test', dLabel))
                    ...
                    util.splitDBTrainValTest(indexes, os.path.join(dirDb1Test, dLabel), dirOutTrainTest1,
                    ...
                    classVecPart['train'], fileNameVecPart['train'] = \
                        util.extractLabels(os.path.join(dirOutTrainTest1, 'train', dLabel), fileNameVec, classVec)
                    classVecPart['val'], fileNameVecPart['val'] = \
                        util.extractLabels(os.path.join(dirOutTrainTest1, 'val', dLabel), fileNameVec, classVec)
                    classVecPart['test'], fileNameVecPart['test'] = \
                        util.extractLabels(os.path.join(dirOutTrainTest1, 'test', dLabel), fileNameVec, classVec)
                    ...
                    # ADP
                    # train
                    adp_train = util.ImageFolderWithPaths(os.path.join(dirOutTrainTest1, 'train'),
                                                               transformP=transform_adp['train'],
                    ...
                    # val
                    adp_val = util.ImageFolderWithPaths(os.path.join(dirOutTrainTest1, 'val'),
                                                             transformP=transform_adp['val'],
                    ...
                    # test
                    adp_test = util.ImageFolderWithPaths(os.path.join(dirOutTrainTest1, 'test'),
                                                             transformP=transform_adp['test'],
                    ...
                    currentModel = functions.warmup(adp_train_loader, adp_val_loader, cnmc_train_loader, cnmc_val_loader,
                                                             currentModel, optimizer_ft,
                                                             dataset1_sizes, dataset2_sizes,
                                                             classVecPart, num_classes,
                                                             cuda, dirResult)
                    ...
                    # train net
                    currentModel = functions.train_model_val(adp_train_loader, adp_val_loader, cnmc_train_loader, cnmc_val_loader,
                                                             cnmc_train, cnmc_val,
                                                             currentModel, optimizer_ft, exp_lr_scheduler,
                                                             dataset1_sizes, dataset2_sizes, batchSize1, batchSize2, classVecPart, num_classes,
